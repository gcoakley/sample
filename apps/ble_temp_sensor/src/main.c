...
#define TEMP_READ_TASK_PRI       (8)
#define TEMP_STACK_SIZE          (64)
static struct os_task temp_read_task;
...
static void init_tasks(void)
{
    ...
    tstack = malloc(sizeof(os_stack_t)*TEMP_STACK_SIZE);
    assert(tstack);

    os_task_init(&temp_read_task, "temp_read_task", readTempTask, NULL, 
            TEMP_READ_TASK_PRI, OS_WAIT_FOREVER, tstack, TEMP_STACK_SIZE);
}
